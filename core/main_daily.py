...
for f in files:
    ...
    simfarm = cultivarModel(cult, region_tol=0.25, metric='Yield',
                            metric_units='t Ha$^{-1}$', extract_flag=True)
    simfarm.train_and_validate_model(nsample=10000, nwalkers=250)
    ...
    with open('../cultivar_models/' + simfarm.cult + '_' + simfarm.metric + '_model_daily.pck', 'wb') as pfile1:
